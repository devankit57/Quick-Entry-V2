...
export default function Home() {
  ...
  const [isWarning, setIsWarning] = useState(false)  // Corrected this line
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case "info":
        return (
          <div className="bg-white/10 backdrop-blur-xl rounded-3xl p-6 mb-8">
            <h2 className="text-2xl font-bold text-white mb-4">About QuickEntry</h2>
            <p className="text-white/80 mb-4">
              QuickEntry is a streamlined registration verification system designed for efficient event management.
            </p>
            <ul className="list-disc list-inside text-white/80">
              <li>Scan QR codes or registration cards</li>
              <li>Manual entry option for backup</li>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white/20 backdrop-blur-xl p-6 rounded-2xl max-w-sm w-full mx-4">
            <div className={`text-2xl ${isWarning ? "text-red-400" : "text-green-400"} mb-4`}>
              {isWarning ? <Info className="w-10 h-10 mx-auto mb-4" /> : <PenTool className="w-10 h-10 mx-auto mb-4" />}
              {modalMessage}
            </div>
            <button
              onClick={() => setIsModalOpen(false)}
              className="w-full bg-gradient-to-r from-purple-600 to-blue-400 text-white font-semibold py-3 rounded-xl"
            >
              Close
            </button>
          </div>
